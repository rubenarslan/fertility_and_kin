...
```{r}
model2_relatives <- brm(person_is_related_man ~ (premenstrual_phase_fab + menstruation + fertile_fab) * hormonal_contraception + (1 + fertile_fab | person), data = diary, backend = "cmdstanr", family = poisson(), cores = 4, file ="brms_fits/relatives_prob4")
...
hypothesis(model2_relatives_seen, "fertile_fab < 0", alpha = 0.01)
hypothesis(model2_relatives_seen, "fertile_fab:hormonal_contraceptionTRUE > 0", alpha = 0.01)

sjPlot::tab_model(model2_relatives_seen)
...
hypothesis(model2_social_life_related_thought_about, "fertile_fab < 0", alpha = 0.01)
hypothesis(model2_social_life_related_thought_about, "fertile_fab:hormonal_contraceptionTRUE > 0", alpha = 0.01)

sjPlot::tab_model(model2_social_life_related_thought_about)
...
### time family curve (all) {.tabset}
```{r}
library(brms)
library(bayesplot)
diary <- readRDS("diary_persons.rds")
diary <- diary %>% filter(reasons_for_exclusion == "" | reasons_for_exclusion == "not_single, ")

model3_family_curve <- brm(time_family ~ s(RCD_fab,  by = hormonal_contraception, bs = "cc") + (1 | person), data = diary, file = "brms_fits/model3_family_curve")

curve_plot(model3_family_curve, "Time spent with family (all)")
