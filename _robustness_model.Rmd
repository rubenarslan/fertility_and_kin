...
if(identical(model$family$family, "gaussian")) {
  ordinal_model = update(model, recompile = TRUE, family = cumulative(), newdata = diary %>% mutate(time_family = as.integer(time_family)) %>% filter(reasons_for_exclusion == ""), file= paste0("brms_fits/", model_prefix, "rob_check_full_ordinal_model"))
...
```{r}
if(outcome != "person_is_related_man") {
  rob_check_nonsingles = update(model, recompile = TRUE, cores = 4, newdata = diary %>% filter(reasons_for_exclusion == "" | reasons_for_exclusion == "not_single, "), file=paste0("brms_fits/", model_prefix, "rob_check_nonsingles"))

summary(rob_check_nonsingles)
}
```
...
```{r mod_fam_inferred}
related_men_inferred <- update(model, recompile = TRUE, formula = person_is_related_man_inferred ~ ., file = paste0("brms_fits/", model_prefix, "related_men_inferred"))
...
if(outcome != "person_is_related_man") {
  robchecks_e[["E11. Include non-singles"]] = rob_check_nonsingles 
}
...
ggsave(paste0("figures/", model_prefix,"_robustness_checks_exclusions.png"), width =10, height = 14)
ggsave(paste0("figures/", model_prefix,"_robustness_checks_exclusions.pdf"), width =10, height = 14)
...
robchecks_p <- list(
  ...
  `M2. No random slopes` = no_varying_slope,
  `M3. Require outcome variance` = with_outcome_var
  )

if(model$family$family == "gaussian") {
  robchecks_p[["M5. Ordinal regression"]] = ordinal_model 
} else {
  robchecks_p[["M4. Gaussian regression"]] = gaussian_model 
}
...
robchecks_p %>% 
  ...
  coord_flip() +
  theme(axis.text.y = element_text(size = 11))
ggsave(paste0("figures/", model_prefix,"_robustness_checks_model.png"), width =10, height = 14)
ggsave(paste0("figures/", model_prefix,"_robustness_checks_model.pdf"), width =10, height = 14)
...
ggsave(paste0("figures/", model_prefix,"_robustness_checks.png"), width =14, height = 14)
ggsave(paste0("figures/", model_prefix,"_robustness_checks.pdf"), width =14, height = 14)
```
