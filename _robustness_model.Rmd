...
ggsave(paste0(model_prefix,"_robustness_checks_exclusions.png"), width =10, height = 14)
...
list(
  ...
  coord_flip() +
  theme(axis.text.y = element_text(size = 10))
ggsave(paste0(model_prefix,"_robustness_checks_model.png"), width =10, height = 14)

# 
# list(
#   `Age moderation` = rob_check_age, 
#   `Weekend moderation` = weekend) %>% 
#   map(~ broom::tidy(.)) %>% 
#   bind_rows(.id = "check") %>% 
#   filter(str_detect(term,"fertile_fab")) %>% 
#   ggplot(aes(paste(model, term), estimate, ymin = conf.low, ymax = conf.high)) + 
#   geom_pointrange() + 
#   coord_flip()
```
